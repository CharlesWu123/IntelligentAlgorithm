...
6. 交叉
   ...
   * 两点交叉：个体编码串中随机设置了两个交叉点，然后再进行部分基因交换
...
7. 变异
   ...
   * 二元变异
   * 高斯变异：进行变异操作时用符号均值为Ｐ，方差为 $P^2$ 的正态分布的一个随机数来替换原有的基因值。
...
8. 选择
   ...
   * 排序选择：对群体中的所有个体按期适应度大小进行排序，基于这个排序来分配各个个体被选中的概率。
   * 最优个体保存：首先按轮盘赌选择方法执行遗传算法的选择操作，然后将当前群体中适应度最高的个体结构完整地复制到下一代群体中
